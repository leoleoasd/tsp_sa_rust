...
fn main() {
    ...
    for _ in 0..threads_count {
        ...
        threads.push(spawn(move || {
            for _ in 0..total_number / threads_count {
                ...
                bar.println(format!(
                    "{} {}",
                    (sa_end_time - begin_time).as_secs_f32(),
                    (dp_end_time - sa_end_time).as_secs_f32()
                ));
                ...
                bar.set_message(format!(
                    "W{}/M{}/H{}/T{}",
                    wrong_count.load(Ordering::SeqCst),
                    medium_count.load(Ordering::SeqCst),
                    huge_wrong_count.load(Ordering::SeqCst),
                    bar.position()
                ));
                bar.inc(1)
            }
    ...
    bar.finish();
    println!(
        "wrong_count: {}, medium_count: {}, huge_wrong_count: {}",
        wrong_count.load(Ordering::SeqCst),
        medium_count.load(Ordering::SeqCst),
        huge_wrong_count.load(Ordering::SeqCst)
    );
